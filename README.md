...
Перечень всех проверяемых рисков содержится в поле `compliance`. Список рисков, поддерживаемых текущей версией API, приведен в таблице ниже.

| Код риска | Описание |
| ------ | ------ |
| `terrorist_person` | В отношении физического лица имеются сведения о причастности к терроризму |
| `terrorist_legal` | В отношении юридического лица имеются сведения о причастности к терроризму |
...
| `disqualified_legal` | Юридическое лицо входит в Реестр дисквалифицированных лиц |
...
| `capital_is_less_than_min` | Организация имеет уставный капитал, размер которого меньше минимально допустимого уставного капитала для данной организационно-правовой формы |
| `min_capital` | Организация имеет уставный капитал, равный минимальному размеру уставного капитала для данной организационно-правовой формы |
| `capital_share_is_owned_by_self` | Доля в уставном капитале принадлежит самой организации |
| `capital_share_is_owned_by_self_1year` | Доля в уставном капитале принадлежит самой организации дольше 1 года |
...
| `cycled_ownership_structure` | В структуре владения организацией присутствуют циклические связи |
| `cant_establish_beneficial_owner` | Организационно-правовая форма организации предполагает наличие бенефициарного владельца, однако его не удалось установить исходя из имеющихся в ЕГРЮЛ сведений об учредителях |
| `recently_registered` | Период деятельности организации с момента регистрации составляет не более 1 года |
...
Проверка всех рисков производится по запрашиваемой организации и всей структуре ее владения "вверх" (учредители компании) и "вниз" (учрежденные компании). Найденные риски находятся в поле `occurrences` соответствующего риска и содержат информацию, которая идентифицирует субъект риска:
